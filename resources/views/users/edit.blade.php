<x-layouts.app>
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-on-surface">Edit User</h1>
                <p class="text-sm text-surface-bright mt-1">Update {{ $user->name }}'s account</p>
            </div>
            <a href="{{ route('users.show', $user) }}" class="text-on-surface-variant hover:text-on-surface text-sm">Back to User</a>
        </div>
        ...
        <div class="bg-surface-container-low rounded-xl border border-outline-variant p-6 max-w-2xl">
            <form method="POST" action="{{ route('users.update', $user) }}" class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-on-surface mb-1">New Password (leave blank to keep current)</label>
                ...
                <div class="flex justify-end gap-3 pt-4">
                    <a href="{{ route('users.show', $user) }}" class="px-4 py-2 text-on-surface-variant hover:text-on-surface rounded-lg text-sm">Cancel</a>
                    <button type="submit" class="px-6 py-2 bg-primary-container text-on-primary-container rounded-lg text-sm font-medium hover:opacity-90 transition-opacity">Update User</button>
                </div>
            </form>
        </div>
    </div>
